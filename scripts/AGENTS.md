...
| File | Description |
|------|-------------|
| `plan_workflow.py` | Planning engine (`VERSION = "0.7.8"`, `SCHEMA_VERSION = 2`). Adapter discovery and sandboxing, isolated A/B agent invocation, evidence/finding validation, frozen external attachments, the `next_action` protocol, synthesis diagnostics, typed adjudication, export and audit export. |
...
`dsh` CLIs or a configured `grounded-build-other-v1` bridge.
