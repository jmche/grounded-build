...
## Purpose
A local coding-agent skill (v0.7.8, public beta) with two independent modes. **Plan** runs two
...
| `VERSION` | Single source of truth for the release version (`0.7.8`). The release gate rejects drift against SKILL.md, README.md, CHANGELOG.md, and `scripts/plan_workflow.py`. |
...
- **Pre-1.0 is enforced, not aspirational.** Bounded compatible refinements stay on the current patch
  line; substantial compatible product capabilities may advance the `0.x.0` minor line. The release
  gate rejects `1.0.0` or above until the maintainer makes an explicit product-readiness decision and
  deliberately changes the gate in that reviewed release.
